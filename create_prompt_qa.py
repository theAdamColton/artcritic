...
"""horse in a spacesuit riding on astronaut - like vehicle. digital painting by kirby"""
...
def main(
    ...
    print('loading model')
    model = LLM(model_url, quantization=quantization, dtype="half", gpu_memory_utilization=0.90, swap_space=16, max_model_len=4096, )
    print('model loaded')
    ...
    while len(output_data) < max_n:
        ...
        while len(chat_templates) < request_batch_size:
            ...
            chat = [
                    ...
                    {"role":"user", "content": prompt_qa + first_user_prompt.format(prompt=example_image_prompt)},
                    {"role":"assistant", "content": example_assistant_response},
                    {"role": "user", "content": final_user_prompt.format(prompt=prompt[:max_prompt_str_len])},
