...
def main(model_args: ModelArgs = ModelArgs()):
    pipeline = model_args.load_model().to(torch.float16).to("cuda")

    im = pipeline(
        "Ken Follett Pillars of the Earth Cathedral Construction, dusty English Summer, Middle Ages, 4K landscape photograph, highly detailed",
