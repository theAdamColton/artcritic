...
class LlavaReward(Reward):
    ...
    def _tokenize(self, assistant_output: str):
        ...
        input_ids_user = llava_utils.tokenizer_image_token(
            user_prompt, self.tokenizer, IMAGE_TOKEN_INDEX, return_tensors="pt"
        )
        ...
        input_ids_assistant = self.tokenizer(
            assistant_response, return_tensors="pt", add_special_tokens=False
        ).input_ids[0]
    ...
    def __call__(self, pixel_values: torch.Tensor, batched_prompt_d):
        ...
        features = [
            ...
            for input_ids, labels in zip(batched_input_ids, batched_labels)
        ]
